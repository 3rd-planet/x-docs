...
the [failResponse](/Overview/opinionated-project-template#failresponse) method is used to format the error message and send it to the user. This method is defined in the methods helper file. The failResponse method takes a message as a parameter and returns an object with the message and a status code. You can change the status code to whatever you want in ``.env``. We use 400 because it is the standard status code for a bad request.
